...
class Engine(Application):
    def __init__(self, raise_exceptions=False):
        logger = getLoggerWithConsoleHandler('hyde-gemini')
    ...
    @command(
        description="hyde-gemini - build hyde sites for gemini",
    ...
    @subcommand(
        'init', help='Initializes hyde-gemini with an exiting hyde site.'
    )
    ...
    def init(self, args):
        """
        The init command. Initializes hyde-gemini with an exiting hyde site
        ...
        copytree(
            Path(__file__).with_name("layout_gemini"),
            dest_path,
            **copy_kwargs,
        )
    ...
    @store('-p', '--port', type=int, default=1965, dest='port',
    ...
    def make_site(self, sitepath, config, deploy=None):
        ...
        config.deploy_root = deploy or Path(sitepath) / "deploy_gemini"
        site = Site(sitepath, config)
        site.context['site'] = site
        site.config.layout_root = getattr(
            site.config, "gemini_layout_root", "layout_gemini"
        )
        return site
